...
public final class RecordingEventListener extends EventListener {
  ...
  /**
   * Removes recorded events up to (and including) an event is found whose class equals {@code
   * eventClass} and returns it.
   */
  ...
  private void logEvent(CallEvent e) {
    ...
    if (startEvent != null) {
      assertThat(eventSequence.contains(startEvent))
          .overridingErrorMessage(e.getName() + " without matching " + startEvent.getName())
          .isTrue();
    }
  ...
  public void connectEnd(
      Call call, InetSocketAddress inetSocketAddress, @Nullable Proxy proxy, Protocol protocol) {
